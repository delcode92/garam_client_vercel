...
useEffect(() => {
  ...
  useEffect(() => {
    fetch("http://103.127.133.115:8080/get_nm_kelompok").then(
...
const handleSelectChangeKecamatan = async (e: { target: { options: any; selectedIndex: any; value: React.SetStateAction<string>; }; }) => {
  ...
  const handleSelectChange = async (e: { target: { value: React.SetStateAction<string>; }; }) => {
    ...
    const response = await fetch('http://103.127.133.115:8080/get_slot_kelompok', {
      method: 'POST',
      headers: {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // SAVE TO DATABASE
    const response = await fetch('http://103.127.133.115:8080/save_petani', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
