...
const Table: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://103.127.133.115:8080/get_rekap_datatable")
    .then(
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Yakin Hapus ?')) {
        // =========== API ACCESS ===========
        const response = await fetch('http://103.127.133.115:8080/del_rekap_datatable', {
          method: 'POST',
          headers: {
  ...
  const handleOpenModal =  async (id: string) => {
    ...
    fetch("http://103.127.133.115:8080/get_edit_rekap_datatable/"+id)
    .then(
      response => response.json()
      )
    .then(
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>)  =>  {
    ...
    // UPDATE DATA REKAP
    const response = await fetch('http://103.127.133.115:8080/update_rekap_datatable', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
